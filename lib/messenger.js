...
class GcpMessenger extends Messenger {
    ...
    constructor() {
        ...
        this.mqttParams = {
          ...
          port: 8883,
          clientId: `projects/${this.gcpParams.projectId}/locations/${region}/registries/${registryId}/devices/${deviceId}`,
          username: 'unused',
          protocol: 'mqtts',
        ...
        if (!process.env.GCP_TOKEN_INTERVAL) {
            process.env.GCP_TOKEN_INTERVAL = 24 * 60
        }
        if (!process.env.GCP_TOKEN_BUFFER) {
            process.env.GCP_TOKEN_BUFFER = 60
        }

        this.defaultDataTopic = `/devices/${deviceId}/events`
    }
    ...
    async connect() {
        ...
        let params = Object.assign({ password: this.createJwt(this.gcpParams.projectId, 'ES256') },
                                   this.mqttParams)
        this.mqtt = await mqtt.connectAsync(params)
        this.connected = true
        setTimeout( async () => { await this.reconnect() }, (process.env.GCP_TOKEN_INTERVAL - process.env.GCP_TOKEN_BUFFER) * 60 * 1000)
        console.log("Connected to IoT Core messaging");
    }
    ...
    createJwt(projectId, secureAlgorithm) {
        const token = {
          ...
          exp: parseInt(Date.now() / 1000) + process.env.GCP_TOKEN_INTERVAL * 60,
          aud: projectId
        }
